...
5. When the priority lane is `deterministic_blockers`, fix every item in `next-task.json`'s `batch` in a single edit. For `must_have_verifiers` and `hillclimb`, keep changes narrow to the primary task only.
