...
from .artifacts import extract_frontmatter, read_text, write_json
...
VERIFIER_SPECS = [
    ...
    ("portfolio_company_disclosure", True, "prompts/verifiers/portfolio_company_disclosure.md"),
    ("research_citation", True, "prompts/verifiers/research_citation.md"),
    ("performance_metrics_disclosure", True, "prompts/verifiers/performance_metrics_disclosure.md"),
...
def build_verifier_requests(draft_path: Path) -> list[VerifierRequest]:
    ...
    for verifier, must_have, prompt_path in VERIFIER_SPECS:
        ...
        requests.append(
            VerifierRequest(
                ...
                instructions="Return only pass or fail with a short rationale.",
            )
        )
    return requests


def write_verifier_bundle(attempt_dir: Path, draft_path: Path) -> dict[str, str]:
    requests = build_verifier_requests(draft_path)
